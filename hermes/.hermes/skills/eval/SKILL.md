...
Run the structural gate with `python3 hermes/.hermes/skills/eval/scripts/validate_dev_cycle.py --baseline-ref HEAD`. Run the behavioral gate with `python3 hermes/.hermes/skills/eval/scripts/run_dev_cycle_behavior.py --baseline-ref HEAD`. A baseline or shared-policy failure is comparative evidence; any failure from a fresh candidate policy is a blocking regression.
...
Run the blocking gate with `python3 hermes/.hermes/skills/eval/scripts/run_subagent_behavior.py --baseline-ref HEAD`. Use `--repetitions 1` only for local runner debugging, never as release evidence.
