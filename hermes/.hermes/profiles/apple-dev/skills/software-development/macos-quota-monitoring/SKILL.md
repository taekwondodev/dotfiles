...
- Diagnose a reported bar-color regression by checking whether it can be a per-subscription branch at all: if every subscription renders through the SAME shared view and ONE shared `color(for:)` call with no subscription switch, the code is per-provider-agnostic and a color difference between two cards cannot come from that view. SwiftUI environment values (`.tint`, `.buttonStyle(.borderedProminent)`'s propagated tint) flow only DOWNWARD to descendants — a prominent button scoped inside one subscription's subtree cannot tint a sibling card's bars. So an asymmetric symptom across cards sharing one code path is not an environment leak and not a per-item branch; verify no alternative `ProgressView`/`Color.accentColor` call sites exist elsewhere in the hierarchy, then treat the cause as a runtime rendering artifact, not a code diff.
- When a reported bar-color recurrence is OLDER than the commit that fixed the same symptom (compare the fix's commit date against the report date), the current code is exonerated: the accent can reappear transiently without a code change (e.g. a recycled `ProgressView` inside `ForEach`/`ScrollView` drawing with the inherited default accent in one frame before its explicit `.tint` lands). Report that there is no diff root cause, and ask the user to reproduce at runtime (expand/collapse the tinted control, drag-reorder) before opening a bug ticket, per the project's rule that environment-tint leaks are caught only by runtime popover inspection.
