...
- **Sign with a stable identity, never ad-hoc, for apps that hold TCC grants (Accessibility, Input Monitoring, Screen Recording).** TCC identifies an app by bundle ID plus signing certificate; an ad-hoc signature is a fresh hash per build, so every `make build` silently voids the grant while the checkbox still looks on. Changing the bundle identifier is a new app even with the same certificate: the grant and login item do not follow the rename. Assemble, sign, and install the new identity first; only then stop the legacy exact executable, remove the legacy bundle, and verify it is gone. A failed compile, sign, or install must leave the former app in place. Removal is a no-op once the legacy path is absent. System Settings remains the only surface for leftover Accessibility and Login Item entries. Resolve the identity in the packaging script from the single `Apple Development` certificate (`security find-identity -v -p codesigning`); fail when zero or more than one match exists rather than falling back to `-`, and report the real signer in `verify` output (parse `Authority=` from `codesign --display --verbose=2`, which prints to stderr). Do not add a self-signed-certificate branch to the script unless the user asks for it: the free Personal Team certificate is enough and one code path keeps `verify` honest. Getting the certificate needs no paid membership: Xcode > Settings > Accounts > add Apple ID > Manage Certificates > `+` Apple Development. When Accounts shows "Failed to retrieve development teams" on an account whose paid membership lapsed, have the user accept any pending agreement on developer.apple.com, then remove and re-add the account; it usually yields the certificate on the second attempt. Only if that still fails, offer a self-signed code-signing certificate from Keychain Access (Certificate Assistant, Self Signed Root, Code Signing, Always Trust) and add it to the script explicitly.
...
- **Stop the process before deleting its bundle.** `rm -rf ~/Applications/<App>.app` while the app runs leaves an orphan process holding the executable with no bundle to quit from; match the exact executable path, terminate those pids, then remove. Applies doubly to a renamed product's leftover bundle and to side-by-side candidate bundles left running from a manual test.
- **A suffix-filtered identity rewrite misses the Makefile and rewrites history.** `Makefile` has no extension, so a `*.swift`/`*.md` pass leaves the help text on the old name. Historical ADRs keep the former product name as written. IOKit class names that happen to contain the old C prefix (`DCPAVServiceProxy`) are not product symbols; leave them alone.
