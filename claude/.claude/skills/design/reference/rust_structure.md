...
One crate per architectural role. Dependency direction hard rule, verified by `cargo build --workspace` — stray import in wrong direction is compile error, not review comment.
...
crates/http             (or the web framework's name) axum/etc. adapter: generic AppState<R, J, ...>,
                        ...
                        crates only — NEVER names an infra-<tech> concrete type. Only the bin
...
Rule of thumb: `domain-*` crates depend on nothing infra/HTTP-flavored, ever. `infra-*`/`http` crates depend on their one `domain-*` crate and nothing else in workspace (never on each other) — shared internal library (health checks, observability primitives) is one external exception every layer allowed to reach for. Bin crate only place allowed to know every concrete type at once.
...
* On shared external dependency with optional features (e.g. `rs-repository-utils`'s `postgres`/`redis`/`health`), enable **only what each crate actually needs** — `domain-<name>` crate should request narrowest feature subset (e.g. `health` only), never inherit blanket feature set across all workspace members. Keeps domain crate infra-agnostic despite technically sharing dependency with infra crates.
* Root package (bin) is simultaneously workspace root and workspace member — this "workspace root that's also a package" layout is normal, not workaround.
