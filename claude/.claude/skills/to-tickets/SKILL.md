...
- Each slice cuts a narrow but COMPLETE path through every layer it touches (Handler → Service → Repository, per `/design`) — vertical, NOT a horizontal slice of one layer
...
Give each ticket its **blocking edges** — the other tickets that must complete before it can start — and its **layer(s)**: which of Handler/Service/Repository/Middleware it touches, and which bounded context. A ticket with no blockers can start immediately.
