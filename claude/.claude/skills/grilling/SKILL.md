...
When the user provides an issue reference, read `writing-for-agents` before writing issue content, then read that issue's full body, comments, and labels through the configured tracker before asking questions. Treat it as the working artifact: preserve its original intent, add the resolved decisions to that same issue's discussion, and hand the same issue to `to-spec` rather than creating a duplicate.
