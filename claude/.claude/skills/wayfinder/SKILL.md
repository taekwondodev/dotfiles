...
5. **Fire the research subagents** in parallel, with one `delegate_task` per research ticket just created, following the coding-standards research flow, capturing findings on a throwaway `research/<name>` branch with a context pointer from the ticket. Each runs isolated and visible in the session, never stealing focus.
